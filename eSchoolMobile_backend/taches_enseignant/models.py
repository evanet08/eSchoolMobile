...
class EleveConduite(models.Model):
    ...
    @staticmethod
    def getMentionConduite(quote):
        return 'Très bien' if(quote==5) else 'Bien' if(quote==4) else 'Satisfaisant' if(quote==3) else 'Mauvaise' if(quote==2) else 'Très mauvaise'    
...
class EleveNote(models.Model):
    ...
    @staticmethod
    def getMentionNote(note,ponderation):
        return 'Assez bon' if(note<ponderation/2) else 'Bon' if(note<ponderation/2+2) else 'Très bon' if(note<ponderation) else 'Excellent'
